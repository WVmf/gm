...
var imMetrics = ['ae', 'pae', 'psnr', 'ssim', 'mae', 'mse', 'rmse', 'mepp', 'fuzz', 'ncc']
var gmMetrics = ['mae', 'mse', 'pae', 'psnr', 'rmse']
...
module.exports = exports = function (proto) {
  function compare(orig, compareTo, options, cb) {
    ...
    if (typeof options === 'object') {
      // Check if a specific metric was specified
      if (options.metric) {
        if (typeof options.metric !== 'string') {
          throw new TypeError('The metric value should be a string');
        }
        var validMetrics = isImageMagick ? imMetrics : gmMetrics;
        metric = options.metric.toLowerCase()
        if (validMetrics.indexOf(metric) == -1) {
          throw new TypeError('Unsupported metric \'' + options.metric + '\'. Supported values: [' + validMetrics.join(', ') + ']');
        }
        args[args.indexOf('mse')] = metric
      }
    ...
    proc.on('close', function (code) {
      ...
      var regex;
      if (isImageMagick) {
        if (['ae', 'psnr', 'ssim', 'ncc'].indexOf(metric) > -1) {
          regex = /(\d+\.?[\d\-\+e]*)/m
        } else { //if (['pae', 'mae', 'mse', 'rmse', 'fuzz'].indexOf(metric) > -1) {
          regex = /\((\d+\.?[\d\-\+e]*)\)/m
        }
      } else {
// --- TODO >>> Do all GM metrics report in the `Total: 0.123` format?
        regex = /Total: (\d+\.?\d*)/m
// --- TODO <<<
      }
